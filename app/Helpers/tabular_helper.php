...
function transform_headers(array $headers, bool $readonly = false, bool $editable = true): string    // TODO: $array needs to be refactored to a new name.  Perhaps $headers?
{
    ...
    foreach ($headers as $element) {    // TODO: This might be clearer to refactor this to `foreach($headers as $header)`
        ...
        $result[] = [
            ...
            'escape'     => !preg_match("/(edit|phone_number|email|messages|item_pic|customer_name|note|mass_bonus)/", key($element)) && !(isset($element['escape']) && !$element['escape']),
...
function customer_headers(): array
{
    return [
        ...
        ['last_name'        => lang('Common.last_name')],
        ['first_name'       => lang('Common.first_name')],
        ['nomorktp'         => lang('Customers.nomorktp')],
        ['email'            => lang('Common.email')],
        ['phone_number'     => lang('Common.phone_number')],
        ['total'            => lang('Common.total_spent'), 'sortable' => false],
        ['saldo'            => lang('Customers.saldo')],
        ['mass_bonus'       => '<input type="checkbox" id="select_all_mass_bonus"> Pilih Penerima Saldo', 'sortable' => false]
    ];
}
 ...
 */
function get_customer_data_row(object $person, object $stats, bool $claimed_this_month = false): array
{
    $controller = get_controller();
    ...
    return [
        ...
        'last_name'        => $person->last_name,
        'first_name'       => $person->first_name,
        'nomorktp'         => $person->nomorktp,
        'email'            => empty($person->email) ? '' : mailto($person->email, $person->email),
        'phone_number'     => $person->phone_number,
        'total'            => to_currency($stats->total),
        'saldo'            => to_currency($person->saldo),
        'mass_bonus'       => '<input type="checkbox" class="mass_bonus_cb" value="' . $person->person_id . '" ' . ($claimed_this_month ? 'disabled title="Sudah claim bulan ini"' : '') . '>',
        'messages'         => empty($person->phone_number)
            ? ''
            : anchor(
                "Messages/view/$person->person_id",
