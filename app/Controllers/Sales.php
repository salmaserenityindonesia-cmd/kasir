...
use App\Models\Sale;
use App\Models\Sales_audit_log;
...
class Sales extends Secure_Controller
{
    ...
    /**
     * Handle Top Up Saldo exclusively from the Sales panel.
     *
     * @return void
     */
    public function postTopup_saldo(): void
    {
        if ($this->request->isAJAX()) {
            $customer_id = $this->request->getPost('customer_id');
            $jumlah_topup = $this->request->getPost('jumlah_topup');

            if (empty($customer_id) || empty($jumlah_topup) || !is_numeric($jumlah_topup) || $jumlah_topup <= 0) {
                echo json_encode(['success' => false, 'message' => 'Input tidak valid. Harap pilih pelanggan dan masukkan nominal yang benar.']);
                return;
            }

            $person_info = $this->customer->get_info($customer_id);

            if (!isset($person_info->person_id) || empty($person_info->person_id)) {
                echo json_encode(['success' => false, 'message' => 'Pelanggan tidak ditemukan.']);
                return;
            }

            $saldo_saat_ini = $person_info->saldo;
            if ($saldo_saat_ini == null) {
                $saldo_saat_ini = 0;
            }

            $saldo_baru = $saldo_saat_ini + $jumlah_topup;

            $builder = $this->customer->builder();
            $builder->where('person_id', $person_info->person_id);
            $success = $builder->update(['saldo' => $saldo_baru]);

            if ($success) {
                $saldo_history = model(\App\Models\Saldo_history::class);
                $employee_id = $this->employee->get_logged_in_employee_info()->person_id;
                $saldo_history->log_topup($person_info->person_id, $employee_id, $jumlah_topup);

                echo json_encode([
                    'success' => true, 
                    'message' => 'Top Up Berhasil! Saldo saat ini: ' . to_currency($saldo_baru)
                ]);
            } else {
                echo json_encode(['success' => false, 'message' => 'Terjadi kesalahan sistem saat memproses top up.']);
            }
        } else {
    ...
    public function postAddPayment(): void
    {
        ...
        if (!$this->validate($rules, $messages)) {
            ...
            } elseif ($payment_type === lang('Sales.deposit')) {
                $customer_id = $this->sale_lib->get_customer();
                $saldo_balance = $this->customer->get_info($customer_id)->saldo;
                $saldo_balance = ($saldo_balance == null ? 0 : $saldo_balance);

                $payments = $this->sale_lib->get_payments();
                $current_payments_with_saldo = isset($payments[$payment_type]) ? $payments[$payment_type]['payment_amount'] : 0;
                $cur_saldo_value = $saldo_balance;

                $amount_tendered = parse_decimals($this->request->getPost('amount_tendered'));

                if (($cur_saldo_value - $current_payments_with_saldo) < $amount_tendered) {
                    $data['error'] = lang('Sales.deposit_remaining_balance') . to_currency($cur_saldo_value - $current_payments_with_saldo);
                } else {
                    $new_saldo_value = $saldo_balance - ($current_payments_with_saldo + $amount_tendered);
                    $new_saldo_value = max($new_saldo_value, 0);
                    $this->sale_lib->set_saldo_remainder($new_saldo_value);
                    $new_saldo_value = str_replace('$', '\$', to_currency($new_saldo_value));
                    $data['warning'] = lang('Sales.deposit_remaining_balance') . $new_saldo_value;
    ...
    public function getRemoveCustomer(): void
    {
        ...
        $this->sale_lib->clear_saldo_remainder();
        $this->sale_lib->delete_payment(lang('Sales.deposit'));
    ...
    public function postComplete(): void    // TODO: this function is huge.  Probably should be refactored.
    {
        ...
        $data['cur_saldo_value'] = $this->sale_lib->get_saldo_remainder();
    ...
    private function _load_customer_data(int $customer_id, array &$data, bool $stats = false): array|string|stdClass|null    // TODO: Hungarian notation
    {
        ...
        if ($customer_id != NEW_ENTRY) {
            ...
            if (!empty($customer_info->company_name)) {
                $data['customer_name'] = $customer_info->company_name;
            } else {
            ...
            $data['tax_id'] = $customer_info->tax_id;
            $data['nomorktp'] = $customer_info->nomorktp;
            $data['saldo'] = $this->sale_lib->get_saldo_remainder() ?? $customer_info->saldo;
        }
    ...
    public function postDelete(int $sale_id = NEW_ENTRY, bool $update_inventory = true): void
    {
        ...
        if (!$has_grant) {
            ...
            $audit_model = model(Sales_audit_log::class);
            foreach ($sale_ids as $id) {
                $old_sale_info = $this->sale->get_info($id)->getRowArray();
                $details = [
                    'action' => 'DELETE',
                    'invoice_number' => $old_sale_info['invoice_number'],
                    'customer_id' => $old_sale_info['customer_id'],
                    'amount_due' => $old_sale_info['amount_due']
                ];
                $audit_model->log_action($id, 'DELETE', $employee_id, $details);
            }
    ...
    public function postSave(int $sale_id = NEW_ENTRY): void
    {
        ...
        $audit_details = [];
        $old_sale_info = $this->sale->get_info($sale_id)->getRowArray();
        $old_payments = $this->sale->get_sale_payments($sale_id)->getResultArray();

        if ($old_sale_info['invoice_number'] != $sale_data['invoice_number']) {
            $audit_details['invoice_number'] = ['old' => $old_sale_info['invoice_number'], 'new' => $sale_data['invoice_number']];
        }

        if ($old_sale_info['customer_id'] != $sale_data['customer_id']) {
            $old_c = $old_sale_info['customer_id'] ? $this->customer->get_info($old_sale_info['customer_id']) : null;
            $new_c = $sale_data['customer_id'] ? $this->customer->get_info($sale_data['customer_id']) : null;
            $audit_details['customer_name'] = ['old' => $old_c ? $old_c->first_name . ' ' . $old_c->last_name : '', 'new' => $new_c ? $new_c->first_name . ' ' . $new_c->last_name : ''];
        }

        if ($old_sale_info['employee_id'] != $sale_data['employee_id']) {
            $old_e = $old_sale_info['employee_id'] ? $this->employee->get_info($old_sale_info['employee_id']) : null;
            $new_e = $sale_data['employee_id'] ? $this->employee->get_info($sale_data['employee_id']) : null;
            $audit_details['employee_name'] = ['old' => $old_e ? $old_e->first_name . ' ' . $old_e->last_name : '', 'new' => $new_e ? $new_e->first_name . ' ' . $new_e->last_name : ''];
        }
        ...
        for ($i = 0; $i < $number_of_payments; ++$i) {
            ...
            foreach ($old_payments as $op) {
                if ($op['payment_id'] == $payment_id) {
                    if ($op['payment_type'] != $payment_type || $op['payment_amount'] != $payment_amount) {
                        $audit_details["payment_{$payment_id}"] = ['old_type' => $op['payment_type'], 'new_type' => $payment_type, 'old_amount' => $op['payment_amount'], 'new_amount' => $payment_amount];
                    }
                    break;
                }
            }
        ...
        if ($payment_type != PAYMENT_TYPE_UNASSIGNED && !empty($payment_amount_new)) {
            $audit_details['payment_new'] = ['type' => $payment_type, 'amount' => $payment_amount_new];
        ...
        if ($this->sale->update($sale_id, $sale_data)) {
            if (!empty($audit_details)) {
                $audit_model = model(Sales_audit_log::class);
                $audit_model->log_action($sale_id, 'EDIT', $employee_id, $audit_details);
            }
