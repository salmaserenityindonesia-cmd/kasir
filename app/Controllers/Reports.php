...
use App\Models\Reports\Detailed_sales_audit;
use App\Models\Reports\Detailed_saldo;
...
class Reports extends Secure_Controller
{
    ...
    private Detailed_sales_audit $detailed_sales_audit;
    private Detailed_saldo $detailed_saldo;
    ...
    public function __construct()
    {
        ...
        $this->detailed_sales_audit = model(Detailed_sales_audit::class);
        $this->detailed_saldo = model(Detailed_saldo::class);
        ...
        if (sizeof($exploder) > 1) {
            ...
            if ($submodule_id === 'saldos' || $submodule_id === 'audits') {
                $submodule_id = 'sales'; // Use sales permission for saldo and audit reports
            }
    ...
    /**
     * Detailed saldo report.
     *
     * @param string $start_date
     * @param string $end_date
     * @return void
     */
    public function detailed_saldo(string $start_date, string $end_date): void
    {
        $this->clearCache();
        ...
        $report_data = $this->detailed_saldo->getData($inputs);
        $summary = $this->detailed_saldo->getSummaryData($inputs);

        $tabular_data = [];
        foreach ($report_data as $row) {
            $tabular_data[] = [
                'id'            => $row['id'],
                'sale_time'     => date('m/d/Y H:i:s', strtotime($row['sale_time'])),
                'customer_name' => $row['customer_name'],
                'employee_name' => $row['employee_name'],
        ...
        $data = [
            'title'        => lang('Reports.detailed_saldo'),
            'subtitle'     => $this->_get_subtitle_report(['start_date' => $start_date, 'end_date' => $end_date]),
            'headers'      => $this->detailed_saldo->getDataColumns(),
            'data'         => $tabular_data,
            'summary_data' => $summary
        ];
    ...
    public function detailed_sales_audit(string $start_date, string $end_date): void
    {
        $this->clearCache();
        ...
        $headers = $this->detailed_sales_audit->getDataColumns();
        $report_data = $this->detailed_sales_audit->getData($inputs);
        ...
        foreach ($report_data as $row) {
            $summary_data[] = [
                ...
                'action_time' => to_datetime(strtotime($row['action_time']))
            ];

            foreach ($row['details'] as $drow) {
                $details_data[$row['id']][] = [
                    $drow['field'],
                    $drow['old_value'],
                    $drow['new_value']
