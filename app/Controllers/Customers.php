...
class Customers extends Persons
{
    ...
    private Saldo_history $saldo_history;
    ...
    public function __construct()
    {
        ...
        $this->saldo_history = model(Saldo_history::class);
    ...
    public function getRow(int $row_id): void
    {
        ...
        $claimed = $this->saldo_history->get_claimed_bonus_this_month([$row_id]);
        $has_claimed = in_array($row_id, $claimed);

        $data_row = get_customer_data_row($person, $stats, $has_claimed);
    ...
    public function getSearch(): void
    {
        ...
        $data_rows = [];
        $customers_result = $customers->getResult();
        
        $person_ids = array_column($customers_result, 'person_id');
        $claimed_this_month = $this->saldo_history->get_claimed_bonus_this_month($person_ids);

        foreach ($customers_result as $person) {
            // Retrieve the total amount the customer spent so far together with min, max and average values
            ...
            $has_claimed = in_array($person->person_id, $claimed_this_month);
            $data_rows[] = get_customer_data_row($person, $stats, $has_claimed);
        }
    ...
    public function postSave(int $customer_id = NEW_ENTRY): void
    {
        ...
        $customer_data = [
            ...
            'nomorktp'          => $this->request->getPost('nomorktp') == '' ? null : $this->request->getPost('nomorktp'),
    ...
    /**
     * Adds mass bonus to selected customers
     */
    public function postAddMassBonus(): void
    {
        $customer_ids = $this->request->getPost('ids');
        ...
        $claimed_this_month = $this->saldo_history->get_claimed_bonus_this_month($customer_ids);
        $employee_id = $this->employee->get_logged_in_employee_info()->person_id;
        $mass_bonus_amount = isset($this->config['mass_bonus_amount']) ? (int) $this->config['mass_bonus_amount'] : 50000;
        $count = 0;

        $updated_saldos = [];
        foreach ($customer_ids as $customer_id) {
            if (!in_array($customer_id, $claimed_this_month)) {
                $customer_info = $this->customer->get_info($customer_id);
                if ($customer_info && $customer_info->person_id) {
                    $new_saldo = $customer_info->saldo + $mass_bonus_amount;
                    
                    // Update saldo
                    $person_data = ['person_id' => $customer_id];
                    $customer_data = ['saldo' => $new_saldo];
                    
                    if ($this->customer->save_customer($person_data, $customer_data, $customer_id)) {
                        $this->saldo_history->log_topup($customer_id, $employee_id, $mass_bonus_amount, 'Top Up Saldo Massal');
                        $updated_saldos[$customer_id] = to_currency($new_saldo);
                        $count++;
                    }
                }
            }
        ...
        if ($count > 0) {
            echo json_encode(['success' => true, 'message' => "Berhasil menambahkan top up saldo massal untuk $count pelanggan.", 'updated_saldos' => $updated_saldos]);
        } else {
            echo json_encode(['success' => false, 'message' => 'Tidak ada topup yang ditambahkan. Mungkin pelanggan yang dipilih sudah menerima topup bulan ini.']);
        }
    }
}
