...
class Sale extends Model
{
    ...
    public function save_value(
        ...
        foreach ($payments as $payment_id => $payment) {
            ...
            } elseif (!empty(strstr($payment['payment_type'], lang('Sales.deposit')))) {
                $cur_saldo_value = $customer->get_info($customer_id)->saldo;
                $new_saldo_value = $cur_saldo_value - $payment['payment_amount'];
                
                $customer_builder = $this->db->table('customers');
                $customer_builder->where('person_id', $customer_id);
                $customer_builder->update(['saldo' => $new_saldo_value]);
                
                $saldo_history = model(\App\Models\Saldo_history::class);
                $saldo_history->log_topup($customer_id, $employee_id, -$payment['payment_amount'], 'Pembayaran Transaksi ' . $sale_id);
            }
    ...
    public function get_payment_options(bool $giftcard = true, bool $reward_points = true): array
    {
        ...
        $payments[lang('Sales.deposit')] = lang('Sales.deposit');
