    ...
    <div class="panel-body">
        ...
        <?php if (isset($customer_id) && $customer_id != -1) { ?>
            <table class="sales_table_100">
                <tr>
                    ...
                    <th style="width: 45%; text-align: right;">
                        <?= anchor("customers/view/$customer_id", $customer_name, ['class' => 'modal-dlg', 'data-btn-submit' => lang('Common.submit'), 'title' => lang('Customers.update')]) ?>
                    </th>
                </tr>
                ...
                <?php } ?>
                <?php if (!empty($nomorktp)) { ?>
                    <tr>
                        <th style="width: 55%;"><?= lang('Customers.nomorktp') ?></th>
                        <th style="width: 45%; text-align: right;"><?= esc($nomorktp) ?></th>
                    </tr>
                <?php } ?>
                <tr>
                    <th style="width: 55%;">Saldo / Deposit</th>
                    <th style="width: 45%; text-align: right;"><?= isset($saldo) ? to_currency($saldo) : to_currency(0) ?></th>
                </tr>
            ...
            <?= anchor(
                ...
                '<span class="glyphicon glyphicon-remove">&nbsp;</span>' . lang('Sales.remove_customer'),
                ['class' => 'btn btn-danger btn-sm', 'id' => 'remove_customer_button', 'title' => lang('Sales.remove_customer')]
            )
            ?>

            <div id="topup_wrapper">
                <button class="btn btn-info btn-sm btn-block" id="btn_show_topup" type="button" style="margin-bottom: 5px;">
                    <span class="glyphicon glyphicon-plus-sign">&nbsp;</span>Top Up Saldo
                </button>
                <div class="form-group form-group-sm" id="topup_controls" style="margin-bottom: 0; display: none;">
                    <label class="control-label col-xs-4" style="text-align: left; padding-right: 0;">Nominal</label>
                    <div class="col-xs-8">
                        <input type="number" id="topup_amount_input" class="form-control input-sm" placeholder="Nominal" min="0" step="any" style="margin-bottom: 5px;">
                        <button class="btn btn-success btn-sm btn-block" id="btn_topup_submit" type="button">Proses Top Up</button>
                    </div>
                </div>
            </div>
        <?php } else { ?>
                    ...
                    <table class="sales_table_100" id="register">
                        ...
                        <tbody id="payment_contents">
                            <?php foreach ($payments as $payment_id => $payment) { ?>
                                <tr>
                                    <td><?= anchor("$controller_name/deletePayment/" . base64_encode($payment_id), '<span class="glyphicon glyphicon-trash"></span>') ?>
                                    </td>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        var clear_fields = function () {
            if ($(this).val().match("<?php //= lang(ucfirst($controller_name) . '.start_typing_item_name') . '|' . lang(ucfirst($controller_name) . '.start_typing_customer_name') ?>")) {
                $(this).val('');
            }
        };
        ...
        $('#btn_show_topup').click(function() {
            $('#topup_controls').slideToggle();
        });

        $('#btn_topup_submit').click(function () {
            var amount = $('#topup_amount_input').val();
            if (amount > 0) {
                $.post("<?= site_url('sales/topup_saldo') ?>", { customer_id: <?= isset($customer_id) ? $customer_id : 0 ?>, jumlah_topup: amount }, function(response) {
                    if (response.success) {
                    ...
                    $.notify({ message: 'Terjadi kesalahan sistem, silakan periksa koneksi atau token Anda.' }, { type: 'danger' });
                });
            } else {
                $.notify({ message: 'Masukkan nominal yang valid.' }, { type: 'warning' });
            }
        });
    });
